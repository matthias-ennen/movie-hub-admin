...
3. Der für das Projekt ermittelte Cloud-Build-Standard ist `612913221205-compute@developer.gserviceaccount.com` und besitzt `roles/editor`. Ihm **keinen** zusätzlichen Zugriff für das Admin-Deploy-Konto zuweisen. Stattdessen `movie-hub-admin-build@movie-hub-62459.iam.gserviceaccount.com` ohne Schlüssel erstellen und dem neuen Konto auf Projektebene **Logs Writer** (`roles/logging.logWriter`), **Artifact Registry Writer** (`roles/artifactregistry.writer`) und **Storage Object Viewer** (`roles/storage.objectViewer`) geben. Die Artifact-Registry- und Storage-Zugriffe können später auf die tatsächlich verwendeten Repositories/Buckets beschränkt werden. Auf genau diesem neuen Build-Dienstkonto dem Admin-Deploy-Konto **Service Account User** (`roles/iam.serviceAccountUser`) erteilen. Der Function-Workflow setzt `--build-service-account` explizit; das breite Compute-Standardkonto bleibt unberührt.
4. Cloud Functions, Cloud Build, Cloud Run und Artifact Registry APIs für `movie-hub-62459` aktivieren. Der erste Deploy-Lauf hat die Function und den Cloud-Run-Dienst `publishannouncement` in `europe-west1` erfolgreich gebaut, ist aber anschließend beim Setzen von `run.services.setIamPolicy` gescheitert. Der Workflow ändert daher künftig **keine** Invoker-IAM-Policy. Ein Projektadministrator erteilt einmalig **auf diesem konkreten Cloud-Run-Dienst** `allUsers` die Rolle **Cloud Run Invoker** (`roles/run.invoker`), beispielsweise mit `gcloud run services add-iam-policy-binding publishannouncement --region=europe-west1 --project=movie-hub-62459 --member=allUsers --role=roles/run.invoker`. Damit darf der Firebase-Client den Callable-Endpunkt erreichen; die Function selbst akzeptiert Veröffentlichungen weiterhin nur mit Firebase-ID-Token und `movieHubAdmin: true`. Anschließend den manuellen Workflow **Deploy Movie Hub Admin** mit `target=functions` erneut ausführen und die Invoker-Policy sowie den Status prüfen. Die normale Movie-Hub-App und die Hosting-Site werden dabei nicht erneut bereitgestellt.

Die Berechtigungsaufteilung folgt der [Firebase-Dokumentation zum Laufzeitkonto](https://firebase.google.com/docs/functions/manage-functions#set_a_service_account), den [Google-Cloud-Deployrollen](https://docs.cloud.google.com/functions/docs/reference/iam/roles#permissions) den [Firestore-IAM-Rollen](https://docs.cloud.google.com/firestore/native/docs/security/iam#predefined_roles) und [Googles Anleitung für eigene Build-Konten](https://docs.cloud.google.com/functions/docs/building#provide_a_service_account_for_building_functions).
