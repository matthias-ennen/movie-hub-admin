...
4. Änderung auf `main` übernehmen, dann den manuellen Workflow **Deploy Movie Hub Admin** mit `target=functions` starten. Die HTTP-Callable muss von Firebase-Clients erreichbar sein, daher `--allow-unauthenticated` auf Transportebene; die Function akzeptiert Veröffentlichungen trotzdem nur mit gültigem Firebase-ID-Token und dem serverseitig geprüften Claim `movieHubAdmin: true`. Für die erste Bereitstellung benötigte API-Aktivierungen und die öffentliche Cloud-Run-Invoker-Bindung anhand des konkreten Deploy-Ergebnisses prüfen. Die separate Hosting-Site und die normale Movie-Hub-App werden hierbei nicht erneut bereitgestellt.
