...
3. Admin-Web-App auf der **bereits angelegten, separaten Hosting-Site** `movie-hub-admin-62459` bereitstellen. Der Hosting-Target `admin` ist in `.firebaserc` eindeutig dieser Site zugeordnet. `npm run build`, dann `firebase deploy --only hosting:admin --project movie-hub-62459`. Die Adresse lautet `https://movie-hub-admin-62459.web.app/`. Falls später OAuth-Anmeldung hinzukommt, die Domain zusätzlich unter Firebase Auth → Autorisierte Domains eintragen.
...
Der manuelle Workflow **Deploy Movie Hub Admin** erwartet einen **eigenen** Pool `movie-hub-admin-github`, darin den OIDC-Provider `github` (Issuer `https://token.actions.githubusercontent.com/`) und das Dienstkonto `github-movie-hub-admin-deploy@movie-hub-62459.iam.gserviceaccount.com`. In diesem Pool `google.subject=assertion.sub` zuordnen und den Provider auf `assertion.repository_owner_id=='208120220' && assertion.repository_id=='1383209480' && assertion.ref=='refs/heads/main'` beschränken. Ausschließlich dem GitHub-OIDC-Subject `repo:matthias-ennen/movie-hub-admin:ref:refs/heads/main` auf **diesem neuen Dienstkonto** die Rolle Workload Identity User erteilen. Dadurch bleiben PR-Builds ohne Firebase-Deployment-Zugriff. Keine dauerhaften Service-Account-Schlüssel anlegen.

Dem neuen Dienstkonto für das erste Hosting-Deployment die Rolle **Firebase Hosting Admin** (`roles/firebasehosting.admin`) zuweisen; diese vordefinierte Rolle gilt auf Projektebene auch für andere Hosting-Sites. Für die Bereitstellung der Cloud Function sind weitere Cloud-Functions-, Build- und Service-Account-Berechtigungen nötig. Sie werden erst für den separaten Workflow-Aufruf `functions` eingerichtet und geprüft, nicht pauschal vom normalen Dienstkonto übernommen. Nach Einrichtung der Identität den PR auf `main` übernehmen und den Workflow zunächst mit `target=hosting`, dann mit `target=functions` ausführen. Der Workflow enthält `--only hosting:admin` beziehungsweise `--only functions:movie-hub-admin` und kann nicht über einen PR-Branch produktiv deployen. Bis dahin hat die Admin-Hosting-Site keinen Release und die Function ist nicht bereitgestellt.
